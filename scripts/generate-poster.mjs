...
// as proportional color bars, stat plate at the foot. Regenerates each build
// from github-static.json. Run: node scripts/generate-poster.mjs
...
const login = gh.login || 'Sami001-OG'
const name = (gh.name || 'sami').toLowerCase()
const year = new Date().getFullYear()
const weeks = gh.contributions?.weeks || []
const totalContribs = gh.contributions?.totalContributions || gh.contributions?.total || 0
const repoCount = (gh.repos?.length || 0) + (gh.pinnedRepos?.length || 0)
...
const stats = [
  { label: 'CONTRIBUTIONS', value: totalContribs, accent: BLUE },
...
<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="${W - M}" y="${M + 74}" text-anchor="end" font-family="'Chivo Mono', monospace" font-size="17" letter-spacing="3" fill="${FAINT}">GITHUB/${login.toUpperCase()}</text>
  ...
  <!-- Contribution field -->
  ${contribCells}
  <text x="${M}" y="${contribY + contribH + 44}" font-family="'Chivo Mono', monospace" font-size="15" letter-spacing="2.5" fill="${FAINT}">FIG 01 — CONTRIBUTION CALENDAR · ${totalContribs} COMMITS/PRS/ISSUES</text>
...
console.log(`✓ Wrote public/poster.png (${W}×${H}) — ${year} annual report`)
